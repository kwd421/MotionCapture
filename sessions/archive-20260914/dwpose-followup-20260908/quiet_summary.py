...
host=[json.loads(l) for l in (root/'quiet-host.jsonl').read_text().splitlines()]
rows=[]
for name in ['quiet-A1','quiet-B1','quiet-B2','quiet-A2']:
 p=root/f'{name}.json'
...
(root/'quiet-summary.json').write_text(json.dumps(rows,indent=2))
