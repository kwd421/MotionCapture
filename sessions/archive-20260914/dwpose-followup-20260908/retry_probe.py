...
def observed(self,packet):
    ...
    try:
        if args.headless:
            ...
            assert frame.identity.sequence == self.frames
            ...
            if self.last_ns is not None:self.intervals.add((returned-self.last_ns)/1e6)
            if self.first_ns is None:self.first_ns=returned
            self.last_ns=returned;self.recent.append(returned);self.frames+=1
        else:
...
if out.exists():
    ...
    report['comparison_arm']='headless_same_fifo_consumer' if args.headless else 'opencv_preview'
    if args.headless and 'preview' in report:
        report['preview']['display']={'backend':'none','window_created':False}
        report['preview']['ui_return_ages']['scope']='scheduled file release to headless consumer; no drawing or display'
        report['preview']['cleanup']='not_opened'
        report['preview']['submitted_frames_scope']='headless consumer packets, not display submissions'
